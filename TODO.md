...
1. **Remove the Health data details card**

   Remove the diagnostic Health data details card from the dashboard now that Withings measurements are confirmed working. Keep normal measurement empty and error states clear without exposing the low-level query details.

2. **Move Health Connect settings into the connection card**

   Move the Health Connect permissions & settings action into the Health Connect connection-status card. Place it beside the Refresh data button when space allows, while keeping both actions usable on narrow screens.

3. **Fix weight, body-fat, and lean-mass reads — verified on device in v0.2.2**
...
4. **Move connection and update controls into Settings — completed in v0.3.0**
...
7. **Add schedule and custom-routine management**
...
8. **Get tracking backup working with Google**
...
9. **Automate dependency updates and commit builds**
...
15. **Add animated in-app branding**
...
17. **Add voice timer announcements**
