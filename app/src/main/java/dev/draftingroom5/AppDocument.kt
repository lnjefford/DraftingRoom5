...
internal fun validateAppDocument(document: AppDocument) {
    ...
    require(document.generation >= 1) { "Generation must be positive." }
...
private fun validateSession(session: GuidedSession, plan: TrainingPlan) {
    ...
    require(session.startedAtMillis >= 0 && session.updatedAtMillis >= session.startedAtMillis && session.eventRevision >= 0) { "Session timestamps or revision are invalid." }
...
private fun validateTimer(timer: SessionTimer, session: GuidedSession) {
    ...
    require(requireNotNull(timer.lastObservedElapsedMillis) >= 0 && requireNotNull(timer.lastHandledCueOrdinal) >= 0) { "Timer progress is invalid." }
